var ef1941 = (function (exports) {
    ...
    const // Antic NTSC palette via https://en.wikipedia.org/wiki/List_of_video_game_console_palettes#NTSC
...
`
    ...
    function setupDisplay(help, mapchrs, units) {
        ...
        putlines('#units', [units], unitcolor, '00', icon, u => `unit-${u.id}`)
            ...
            .each(function(u) {
                ...
                u.blink = function() {
                    select(`#unit-${this.id}`).classed('blink', true);
                };
    ...
    */
    ...
    function unfocusUnit() {
        ...
        selectAll('.blink').classed('blink', false);
    ...
    function stopUnitsFlashing() {
        selectAll('#units .chr-fg.flash').classed('flash', false);
    }
    ...
    function toggleZoom() {
        ...
        select('#map-window .container').classed('doubled', gameState.zoom);
        elt.scrollIntoView({block: "center", inline: "center"});
    }

    function toggleExtras() {
        gameState.extras = !gameState.extras;
        selectAll('.extra').style('visibility', gameState.extras ? 'visible': 'hidden');
    }

    function toggleDebug() {
        gameState.debug = !gameState.debug;
        selectAll('.debug').style('visibility', gameState.debug ? 'visible': 'hidden');
    }
    ...
    */
    ...
    function mapinfo(ev, m) {
        ...
        select(this).attr('title', clauses.join('\n'));
    }
    ...
    function toggleHelp() {
        ...
        select('#help-window').style('display', gameState.help ? 'block': 'none');
        select('#map-scroller').style('display', gameState.help ? 'none': 'block');
