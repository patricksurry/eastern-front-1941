...
const
    ...
    loc34 = mapboard.locationOf(new Location(3, 4));

test("Invalid lon arg should throw", () => {
    expect(() => new Location({lon: 3, lat: 4})).toThrow();
})
...
const finn2 = mapboard.locationOf(new Location(35, 38)),
    costs = moveCosts(UnitKind.infantry, Weather.dry); // oob.at(42)

test("Unexpected bestPath() for 2 Finn Inf", () => {
    expect(mapboard.bestPath(finn2, new Location(36, 33), costs).orders.length).toBe(10);
})
